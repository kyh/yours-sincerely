...
export const ShareButton = ({ post }: Props) => {
  ...
  // Opening tries the native share sheet first; the menu is its fallback.
  ...
  return (
    <ResponsiveMenu open={isOpen} onOpenChange={handleOpenChange}>
      <ResponsiveMenuTrigger className="hover:bg-accent size-8 cursor-pointer rounded-lg p-2 transition">
        <span className="sr-only">Share post</span>
        <ShareIcon className="size-4" />
      </ResponsiveMenuTrigger>
      <ResponsiveMenuContent
        title="Share Post"
        description="Share this tiny beautiful letter with the world"
      >
        <ResponsiveMenuLinkItem
        ...
        <ResponsiveMenuLinkItem
          href={`https://x.com/intent/post?url=${encodedPostUrl}`}
          target="_blank"
          rel="noopener noreferrer"
        >
          <svg
        ...
        <ResponsiveMenuItem onClick={copyLink}>
