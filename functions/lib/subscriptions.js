...
exports.updateStripeSubscription = (change, context) => {
  ...
  if (!change.after.exists) {
    console.log(
      `deleted subscription ${change.before.id}: ${change.before.data()}`
    );
  ...
  console.log(
    `update subscription ${subscription.id} with ${subscription.tempStripePaymentTokenId}`
  );
  return getUser(subscription.createdBy)
    ...
    .then((stripeCustomerId) =>
      updateStripeCustomerPaymentMethod(
        stripeCustomerId,
        subscription.tempStripePaymentTokenId
      )
    )
    .then((stripeCustomer) =>
      createOrGetStripeSubscription(
        stripeCustomer.id,
        subscription.stripeSubscriptionId
      )
    )
    .then((stripeSubscription) =>
      saveStripeSubscriptionToDatabase(subscription.id, stripeSubscription)
    )
    .catch((error) => {
...
const createOrGetStripeCustomerId = (user) => {
  if (user.stripeCustomerId) {
    ...
    return getUserEmail(user.id)
      ...
      .then((stripeCustomer) =>
        saveStripeCustomerAndReturnId(user, stripeCustomer)
      );
  }
};
const saveStripeCustomerAndReturnId = (user, stripeCustomer) => {
  return saveStripeCustomerIdToDatabase(user.id, stripeCustomer.id).then(
    () => stripeCustomer.id
  );
};
...
const saveStripeSubscriptionToDatabase = (
  subscriptionId,
  stripeSubscription
) => {
  return admin
